...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verify_csrf_token($csrfToken)) {
        ...
        if ($pdo) {
            $stmt = $pdo->prepare("SELECT id, username, password FROM parents WHERE username = :username");
...
?>
    ...
    <section class="connexion-section">
        <div class="form-container">
            <h3>Connexion Parents</h3>

            <?php if ($error): ?>
                <p class="error-message"><?= h($error) ?></p>
            <?php endif; ?>

            <form action="connexionparents.php" method="POST" class="login-form">
            ...
            <div class="form-footer">
                <p>Pas encore de compte ? <a href="inscriptionparents.php">S'inscrire</a></p>
            </div>
        </div>
    </section>
