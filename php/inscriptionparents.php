...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verify_csrf_token($csrfToken)) {
        ...
        if ($pdo) {
            ...
            $stmt = $pdo->prepare("SELECT id FROM profs WHERE username = :username OR email = :email");
            $stmt->bindParam(':username', $username, PDO::PARAM_STR);
            $stmt->bindParam(':email', $email, PDO::PARAM_STR);
            $stmt->execute();

            if ($stmt->fetch()) {
                $error = "Nom du professeur ou email déjà utilisé.";
            } else {
                // Insérer l'utilisateur avec hashage sécurisé
                $hashedPassword = password_hash($password, PASSWORD_DEFAULT);
                $stmt = $pdo->prepare("INSERT INTO parents (username, password, email) VALUES (:username, :password, :email)");
                $stmt->bindParam(':username', $username, PDO::PARAM_STR);
...
?>
    ...
    <section class="connexion-section">
        <div class="form-container">
            ...
            <form action="inscriptionparents.php" method="POST" class="login-form">
            ...
            <div class="form-footer">
                <p>Déjà un compte ? <a href="connexionparents.php">Se connecter</a></p>
            </div>
        </div>
    </section>
